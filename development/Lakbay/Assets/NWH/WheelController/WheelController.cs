...
namespace NWH.WheelController3D
{
    ...
    public class WheelController : MonoBehaviour
    {
        ...
        /// <summary>
        ///     Should the movement of the wheel visual be smoothed?
        /// </summary>
        ...
        public void OnPrePhysicsSubstep(float t, float dt)
        {
            ...
            if (hasHit && _hasHitARigidbody && applyForceToOthers)
        ...
        public void OnPhysicsSubstep(float t, float dt, int i)
        {
            ...
            // Add chassis torque
            if (squat != 0 && forwardFriction.force > 0) _chassisTorque += forwardFriction.force * wheel.radius * squat;

            _totalForceThisUpdate += _surfaceForceVector * (dt / t);
        }
        ...
        public void OnPostPhysicsSubstep(float t, float dt)
        {
            if (!_hasBeenEnabledThisFrame)
                _chassisTorque += (wheel.angularVelocity - wheel.prevAngularVelocity) * wheel.inertia / dt;

            if (_chassisTorque < -0.1f || _chassisTorque > 0.1f)
            {
                Vector3 torqueForce = wheel.forward * (_chassisTorque * 0.5f);
                parentNRigidbody.AddForceAtPosition(-torqueForce, wheel.worldPosition + wheel.up);
                parentNRigidbody.AddForceAtPosition(torqueForce,  wheel.worldPosition - wheel.up);
            }
        ...
        {
            if (!useCurve) return 11000f * (1f - Mathf.Exp(-0.00014f * load));
        ...
        {
            if (!useCurve) return 18000f * (1f - Mathf.Exp(-0.0001f * load));
        ...
        public void UseExternalUpdate()
        {
            ...
            if (_usingExternalUpdate) return;
        ...
        {
            // Objects
            if (parent == null || reset) parent = FindParent();
            ...
            if (parentNRigidbody == null) parentNRigidbody = parent.AddComponent<NRigidbody>();

            Debug.Assert(parentNRigidbody != null, "Parent does not contain a NRigidbody.");

            if (frictionModel == null || reset) frictionModel = new StandardFrictionModel();
            ...
            // Assign layer mask
            if (autoSetupLayerMask && parent != null) SetupLayerMask();
        ...
        public void InitializeScanParams()
        {
            ...
            for (float x = _boundsX; x <= _boundsZ; x += _stepX)
            {
                ...
                for (float y = _boundsY; y <= _boundsW; y += _stepY)
                {
                    ...
                    float xOffset                           = x;
                    if (lateralScanResolution == 1) xOffset = 0;
        ...
        private void HitUpdate()
        {
            ...
            if (autoSetupLayerMask) Physics.queriesHitTriggers = false;
            ...
            else
            {
                ...
                if (hasHit) CalculateAverageWheelHit();
            }
            ...
            if (hasHit)
            {
                ...
                if (_hasHitARigidbody) _surfaceVelocity = _hitRigidbody.GetPointVelocity(wheelHit.point);
            }
            else
            {
                _surfaceVelocity  = Vector3.zero;
            ...
            if (autoSetupLayerMask) Physics.queriesHitTriggers = initQueriesHitTriggers;
        ...
        private void SuspensionUpdate()
        {
            ...
            else
            {
                ...
                spring.length = Mathf.Lerp(spring.length, spring.maxLength, _fixedDeltaTime * 6f);
                damper.force  = 0;
            }

            if (_hasBeenEnabledThisFrame) spring.prevLength = spring.length;
            ...
            Vector3 suspensionForce = suspensionForceMagnitude * surfaceNormal;
            parentNRigidbody.AddForceAtPosition(suspensionForce,
                                                _transformPosition, false);

            _totalForceThisUpdate += suspensionForce;
        }
        ...
        private void WheelUpdate()
        {
            ...
            if (hasHit) wheelHit.force = wheel.load;
            ...
            if (smoothMovement && hasHit && !spring.bottomedOut && !spring.overExtended)
            {
                ...
                wheel.smoothYPosition = Mathf.Lerp(wheel.smoothYPosition, localPos.y,
                                                   _fixedDeltaTime * 5f + absSpringVel * 0.2f);
        ...
        {
            if (spring.length > 0 || wheelHit == null) return;
            ...
            Vector3 contactVelocityVector =
                parentNRigidbody.velocity - (rigidbodyBIsNull ? Vector3.zero : rigidbodyB.velocity);
            float contactVelocity = Vector3.Dot(contactVelocityVector, normal);

            float f = parentNRigidbody.mass / vehicleWheelCount * -Physics.gravity.y * (1f / wheel.radius);
            float j = penetration * f * 12f * _fixedDeltaTime +
                      -contactVelocity * _fixedDeltaTime * f * (contactVelocity < 0 ? 0.8f : 3f);
            Vector3 fullImpulse = normal * j;

            parentNRigidbody.AddForceAtPosition(fullImpulse / _fixedDeltaTime, transform.position);

            if (rigidbodyB != null) rigidbodyB.AddForceAtPosition(-fullImpulse, point, ForceMode.Impulse);
        }
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                if (wheelRay.valid)
                {
                    ...
                    if (_weight < minWeight)
                        minWeight                           = _weight;
                    else if (_weight > maxWeight) maxWeight = _weight;
        ...
        public Side DetermineSide(Vector3 pointPosition, Transform referenceTransform)
        {
            ...
            if (relativePoint.x < 0.0f) return Side.Left;
        ...
        private void SetupLayerMask()
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                if (layers.All(l => l != layer)) layers.Add(layer);
            }
        ...
        private void GetVehicleColliders(Transform parent, ref List<GameObject> colliderGOs)
        {
            ...
            {
                if (collider.gameObject.layer == 0) collider.gameObject.layer = 2;
